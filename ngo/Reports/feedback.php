...
$result = $database->runQuery("select f.feedback,f.createddate, u.first_name from feedback f join user u where f.user_id=u.user_id");
$header = $database->runQuery("SELECT UCASE(`COLUMN_NAME`) 
FROM `INFORMATION_SCHEMA`.`COLUMNS` 
WHERE `TABLE_SCHEMA`='outdoor_kings' 
AND `TABLE_NAME`='feedback'
and `COLUMN_NAME` in ('feedback','createddate','user_id')");
